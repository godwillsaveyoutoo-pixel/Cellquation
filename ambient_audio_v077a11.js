/* Cellquation v0.7.7a.11 — persistent nature playlist + deterministic carousel navigation.
...
(()=>{
  ...
  let uiBound=false, scrollTimer=0, requestedIndex=null, userScrollIntent=false, suppressScrollSelection=false;
  ...
  async function explicitTrackChange(index,phase=0,{autoplay=true,scroll=true}={}){
    ...
    }finally{changingTrack=false;syncControls()}
  }
    ...
    if(carousel){
      ...
      carousel.addEventListener('scroll',()=>{
        ...
        clearTimeout(scrollTimer);scrollTimer=setTimeout(()=>{
          ...
          cards.forEach(c=>{const d=Math.abs((c.offsetLeft+c.offsetWidth/2)-center);if(d<dist){best=c;dist=d}});
  ...
  function scrollToTrack(i,smooth=true){
    const carousel=document.querySelector('[data-cq-track-carousel]');const card=carousel?.querySelector(`[data-cq-track-index="${i}"]`);if(!carousel||!card)return;
    ...
    // Keep suppression through the entire smooth-scroll animation.
