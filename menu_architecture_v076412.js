...
function ratioNode(goal){const r=el('div','cq-ratio');(goal||[]).forEach((v,i)=>{if(i)r.append(el('span','cq-ratio__colon',':'));r.append(el('span',`cq-ratio__part is-${['blue','green','violet'][i]||'blue'}`,String(v)))});return r}
...
function cardArt(kind,colour){
  const blue='<img class="cq-cell-mini" src="assets/ui/hud_goal_blue.png?v=0.7.7a.8" alt="">';
  const second=colour==='three'?'<img class="cq-cell-mini secondary" src="assets/ui/hud_goal_violet.png?v=0.7.7a.8" alt="">':'<img class="cq-cell-mini secondary" src="assets/ui/hud_goal_green.png?v=0.7.7a.8" alt="">';
  if(kind==='network')return `${blue}<span class="cq-link-mini"></span>${second}`;
  return blue;
}
...
async function browser(){
  ...
  document.getElementById('backLink').href=`index.html?mode=${modeQuery(cfg.colour)}`;
