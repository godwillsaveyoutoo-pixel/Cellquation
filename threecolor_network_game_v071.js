...
function transformNetwork(){
  ...
  if(portrait&&rawMaxY>.001){
    ...
    // v0.7.2: calm family-specific portrait fill. Flat topologies get room to
    // breathe vertically; already-tall/radial grammars stay close to authored proportions.
    const family=authored?.compositionFamily||'';
    ...
    const authoredStretchCap=Math.max(Number(authored?.portraitYStretchMax||1),Number(familyCaps[family]||1));
    const yStretch=clamp(availableHalfY/rawMaxY,1,authoredStretchCap);
...
function startFusion(anchor,source){const a=occupancy[anchor],b=occupancy[source];if(!a||!b||a.type!=='fusion'||b.type!=='fusion'||!adjacent(anchor,source))return false;const resultSpecies=fusionResultSpecies(a.species,b.species);if(!resultSpecies)return false;active=new EdgeTransport({sourceNode:source,targetNode:anchor,species:resultSpecies,sourceSpecies:b.species,targetSpecies:a.species,resultSpecies,mode:'fusion'});active.context='fusion';runtime.mode='acting';runtime.moves++;hint.textContent=a.species===b.species?'Same-colour Fusion.':'Mixed Fusion creates the third colour.';clearSelection();return true}
...
function handleNode(i){if(runtime.mode!=='playing'||runtime.paused)return;const c=occupancy[i],s=runtime.selectedNode,src=s!=null?occupancy[s]:null;
  ...
  if(!c)return;
  if(c.type==='fusion'){if(neighbors(i).some(j=>occupancy[j]?.type==='fusion')){runtime.selectedNode=i;runtime.targetMode='fusion'}else hint.textContent='No connected Fusion partner.'}
...
function completeLevel(){if(runtime.mode==='complete')return;runtime.mode='complete';const stars=starsForMoves(runtime.moves);progress=recordCompletion('threenetwork',progress,{levelId:level.id,levelIndex:runtime.levelIndex,moves:runtime.moves,stars,levelCount:campaign.levels.length});starsEl.textContent='★'.repeat(stars)+'☆'.repeat(3-stars);starsEl.setAttribute('aria-label',`${stars} of 3 stars`);resultSummary.textContent=`${runtime.moves} ${runtime.moves===1?'MOVE':'MOVES'} · OPTIMAL ${level.min}`;const best=progress.best[level.id];bestSummary.textContent=`BEST ${best.moves} MOVES · ${best.stars}/3 ★`;continueBtn.textContent=runtime.levelIndex<campaign.levels.length-1?'NEXT LEVEL':'CAMPAIGN COMPLETE';window.CellquationSFX?.success();window.CellquationUI?.showResult(resultBackdrop,continueBtn)}
...
async function init(){
  [synapseCanonical,campaign,layoutPack,visualConfig]=await Promise.all([
    ...
    fetch('./content/threecolor/LIVING_NETWORKS_FULL_48_V071.json').then(r=>r.json()),
    fetch('./content/threecolor/LIVING_NETWORKS_LAYOUT_FULL_48_V071.json').then(r=>r.json()),
